    ...
    if (!$email && !$password) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if ($user['role'] == 'admin') {
                ...
                exit(); // Keluar dari script setelah mengalihkan header
            } elseif ($user['role'] == 'user') {
                header('Location: indexUser.php?dashboardUser');
                exit(); // Keluar dari script setelah mengalihkan header
            }
        } else {
...
if (isset($_GET['delete_staff'])) {
    $room_id = $_GET['delete_staff'];
    $sql1 = "DELETE FROM emp_history WHERE emp_id = '$room_id'";
    $sql2 = "DELETE FROM staff WHERE emp_id = '$room_id'";
...
if (isset($_POST['createComplaint'])) {
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:indexUser.php?complain&success");
    } else {
        header("Location:indexUser.php?complain&error");
    }

}

if (isset($_POST['createComplaintAdmin'])) {
    $complainant_name = $_POST['complainant_name'];
    $complaint_type = $_POST['complaint_type'];
    $complaint = $_POST['complaint'];
